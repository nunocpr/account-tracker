...
import { fetchMainCategories } from "@/app/_lib/db/mainCategoryFunctions";
        ...
        if (!userId) {
            return NextResponse.json(
                { error: "Unauthorized" },
                {
                    status: 401,
                    statusText:
                        "You must be logged in to create a new main category.",
                }
            );
        }
        ...
        if (type === ActionType.Add) {
            ...
            if (existingMainCategory) {
                return NextResponse.json(
                    ...
                    { status: 400, statusText: "Main category already exists" }
                );
            }
